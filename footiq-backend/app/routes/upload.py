...
async def upload_match_data(match_id: str, file: UploadFile = File(...)):
    # #region agent log
    debug_log("initial-run", "H1", "upload.py:upload_match_data:start", "Upload endpoint hit", {"match_id": match_id, "filename": file.filename})
    # #endregion
    ...
    try:
        ...
        debug_log(
            "post-fix",
            "H1",
            "upload.py:upload_match_data:handled_error",
            "Upload failed with handled value error",
        ...
        debug_log(
            "post-fix",
            "H1",
            "upload.py:upload_match_data:unexpected_error",
    ...
    debug_log(
        "initial-run",
        "H1",
        "upload.py:upload_match_data:success",
        "Upload indexed successfully",
        {"match_id": match_id, "events_loaded": len(events), "chunks_created": chunks_created},
    )
    # #endregion
